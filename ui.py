...
class UI:
    ...
    def __init__(self, simulator: Simulator, renderers: Sequence[Renderer], ui_scale: Optional[float] = None) -> None:
        ...
        self.ui_scale = self.UI_SCALE if ui_scale is None else ui_scale
    ...
    def _draw_menu_bar(self) -> None:
        ...
        pygame.draw.rect(self.canvas, renderer_color, self.renderer_button_rect)
        if self.open_menu == "renderer" or renderer_hover:
            pygame.draw.rect(self.canvas, self.MENU_BORDER, self.renderer_button_rect, width=1)
        self.canvas.blit(renderer_label, renderer_rect)
    ...
    def _draw_hints(self) -> None:
        hint_text = (
            ...
            "( ): gain | i: invert | +/-: scale | n: new node | k: kill random node"
        )
